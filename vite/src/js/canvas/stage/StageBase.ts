...
export class StageBase {
	...
	private debugFlg: boolean = false;
	...
	public drowHVMainContainer: DrowContainerBase;
	...
	constructor(canvasId: string) {
		...
		this.drowHVMainContainer = new DrowContainerBase();		//
		...
		this.stage.addChild(this.drowHVMainContainer);
	 ...
	 * @memberof StageBase
	 */
	public setMainContenarYPos(yPos:number){
		let canvasSize:Utils.Size = this.getCanvasSize();
	...
	public mousedownEvent(evt) {
		...
		this.mousedownEventSub(this.drowMainContainer);
		this.mousedownEventSub(this.drowVMainContainer);
		this.mousedownEventSub(this.drowHMainContainer);
		this.mousedownEventSub(this.drowHVMainContainer);
		this.mousedownEventSub(this.drowFrontContainer);
		this.mousedownEventSub(this.drowSystemContainer);
		this.mousedownEventSub(this.drowDebugContainer);
	...
	public mousemoveEvent(evt) {
		...
		if(this.holdParam.uuid == 'WORLD' && this.holdParam.flg){
			...
			this.mousemoveEventSub(this.drowHVMainContainer);
	...
	public mouseupEvent(evt) {
		if(this.holdParam.flg || this.selectDownParam.flg || this.selectUpParam.flg){
			this.mouseupEventSub(this.drowGroundContainer);
			...
			this.mouseupEventSub(this.drowHVMainContainer);
	...
	public update(evt) {
		...
		this.updateSub(this.drowHVMainContainer);
	...
	public getInputDisp():string{
        ...
        entity.taskName       = "";
        entity.progress       = 0;
        entity.planPeriodFrom = new Date();
        entity.planPeriodTo   = new Date();
        entity.memo           = "";

		let key:string;
		for(key in entity){
			let tag:HTMLInputElement;
			tag = document.querySelector("#" + key) as HTMLInputElement;
			if(Utils.isNotEmpty(tag)){
				if(key == 'planPeriodFrom' || key == 'planPeriodTo'){
					let str = tag.value;
					entity[key] = new Date(str);
				} else if(key == 'progress'){
					entity[key] = parseFloat(tag.value) / 100.0;
				}else{
					entity[key] = tag.value;
				}
			}
		}
	...
	public setInputDisp(inputID:string, jsonStr:string){
		...
		for(key in json){
			...
			if(Utils.isNotEmpty(tag)){
				if(key == 'planPeriodFrom' || key == 'planPeriodTo'){
					let str = json[key];
					tag.value = str.substr(0,10);
				} else if(key == 'progress'){
					let tProg = Math.round(parseFloat(json[key]) * 10000.0);
					tag.value = (tProg / 100.0).toString();
				}else{
					tag.value = json[key];
				}
			}
