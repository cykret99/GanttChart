...
export class DrowTaskBar extends DrowContainerBase {
    ...
    public mouseHoldUpEvent(gPos: createjs.Point, wPos: createjs.Point, pPos: createjs.Point, lPos: createjs.Point, stage: StageBase): void {
        ...
        // 現在描画しているオブジェクトの場合、描画を更新
        let entity = new TaskBarEntity;
        entity.dispToEntity(stage.getInputDisp());
        entity.planPeriodFrom = this.planPeriod.fromDate;
        entity.planPeriodTo = this.planPeriod.toDate;
        stage.setInputDisp("main", JSON.stringify(entity));

        let body = new FormData()
        body.append('bridgeBox', JSON.stringify(entity));
        RestAPI.client.post(function(eve){;}, CanvasConst.getContext() + "home/updatePlanDate", body,
        {
